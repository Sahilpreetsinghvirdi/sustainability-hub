  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <Stack flexDirection="row" justifyContent="space-between" alignItems="center" marginBottom="xl">
        ...
        <Text fontSize="20" fontWeight="700" color="#0A0A0A">Household</Text>
      ...
      <Card variant="elevated" padding="lg" marginBottom="lg" style={styles.householdCard}>
        <Stack flexDirection="row" alignItems="center" gap="3" marginBottom="lg">
          <Stack width={56} height={56} borderRadius="lg" backgroundColor="rgba(10,10,10,0.12)" alignItems="center" justifyContent="center">
            <Ionicons name="home" size={28} color="#1C1C1C" />
          </Stack>
          <Stack flex={1}>
            {isEditing ? (
              ...
              <>
                <Text fontSize="20" fontWeight="700" color="#0A0A0A">{household.name}</Text>
                <Text fontSize="8" color="#444444">Created {new Date(household.created_at).toLocaleDateString()}</Text>
              </>
            )}
          </Stack>
          {isEditing ? (
        ...
        <Stack flexDirection="row" gap="3">
          <Stack flex={1} alignItems="center" style={styles.statItem}>
            <Text fontSize="20" fontWeight="800" color="#0A0A0A">{household.size}</Text>
            <Text fontSize="8" color="#444444">Members</Text>
          </Stack>
          <Stack flex={1} alignItems="center" style={styles.statItem}>
            <Text fontSize="20" fontWeight="800" color="#6B6B6B">{household.total_scans}</Text>
            <Text fontSize="8" color="#444444">Total Scans</Text>
          </Stack>
          <Stack flex={1} alignItems="center" style={styles.statItem}>
            <Text fontSize="20" fontWeight="800" color="#1C1C1C">{household.avg_carbon_per_person}</Text>
            <Text fontSize="8" color="#444444">kgCOâ‚‚e/person</Text>
          </Stack>
        </Stack>
      </Card>
      ...
      <Card variant="default" padding="lg" marginBottom="lg">
        <Text fontSize="16" fontWeight="600" color="#0A0A0A" marginBottom="lg">Household Settings</Text>

        <Stack gap="4">
          {isEditing && (
            <>
              <Stack>
                <Text fontSize="12" fontWeight="600" color="#0A0A0A" marginBottom="1">Household Size</Text>
              ...
              <Input
                ...
                leftIcon={<Ionicons name="location" size={20} color="#444444" />}
              />

              <Stack>
                <Text fontSize="12" fontWeight="600" color="#0A0A0A" marginBottom="1">Home Type</Text>
                <Stack flexDirection="row" gap="2">
                  {homeTypes.map(type => (
                    <Button
                      key={type.value}
          ...
          {!isEditing && (
            <Stack gap="3">
              <Stack flexDirection="row" justifyContent="space-between">
                <Text fontSize="12" color="#444444">Size</Text>
                <Text fontSize="12" fontWeight="600" color="#0A0A0A">{household.size} people</Text>
              </Stack>
              <Stack flexDirection="row" justifyContent="space-between">
                <Text fontSize="12" color="#444444">ZIP Code</Text>
                <Text fontSize="12" fontWeight="600" color="#0A0A0A">{household.zip_code || 'Not set'}</Text>
              </Stack>
              <Stack flexDirection="row" justifyContent="space-between">
                <Text fontSize="12" color="#444444">Home Type</Text>
                <Text fontSize="12" fontWeight="600" color="#0A0A0A">{household.home_type}</Text>
              </Stack>
            </Stack>
          )}
        </Stack>
      ...
      <Card variant="default" padding="lg" marginBottom="lg">
        <Stack flexDirection="row" justifyContent="space-between" alignItems="center" marginBottom="lg">
          <Text fontSize="16" fontWeight="600" color="#0A0A0A">Members</Text>
        ...
        <Stack gap="3">
          {household.members.map((member: any) => (
            <Stack key={member.id} flexDirection="row" alignItems="center" gap="3" style={styles.memberItem}>
              <Avatar size="md" backgroundColor="#1C1C1C">
                {member.avatar}
              </Avatar>
              <Stack flex={1}>
                <Text fontSize="12" fontWeight="600" color="#0A0A0A">{member.name}</Text>
                <Text fontSize="8" color="#444444">{member.email}</Text>
              </Stack>
              ...
              {member.role !== 'owner' && (
                <Button variant="ghost" size="xs" onPress={() => handleRemoveMember(member.id)}>
                  <Ionicons name="close-circle" size={20} color="#444444" />
                </Button>
              )}
            </Stack>
          ))}
      ...
      <Card variant="default" padding="lg" marginBottom="lg" style={styles.dangerCard}>
        <Stack flexDirection="row" alignItems="center" gap="3" marginBottom="lg">
          <Stack width={40} height={40} borderRadius="lg" backgroundColor="rgba(10,10,10,0.12)" alignItems="center" justifyContent="center">
            <Ionicons name="warning" size={22} color="#444444" />
          </Stack>
          <Stack flex={1}>
            <Text fontSize="16" fontWeight="600" color="#444444">Danger Zone</Text>
            <Text fontSize="8" color="#444444">Irreversible actions</Text>
          </Stack>
        </Stack>
        ...
        <Stack gap="3">
          <Button variant="outline" fullWidth onPress={() => Alert.alert('Leave Household', 'Are you sure?')} style={styles.dangerButton}>
            <Stack flexDirection="row" alignItems="center" justifyContent="center" gap="2">
              <Ionicons name="exit" size={20} color="#444444" />
              <Text style={{ color: '#444444' }}>Leave Household</Text>
            </Stack>
          </Button>
          <Button variant="outline" fullWidth onPress={() => Alert.alert('Delete Household', 'This cannot be undone')} style={styles.dangerButton}>
            <Stack flexDirection="row" alignItems="center" justifyContent="center" gap="2">
              <Ionicons name="trash" size={20} color="#444444" />
              <Text style={{ color: '#444444' }}>Delete Household</Text>
            </Stack>
          </Button>
        </Stack>
      </Card>
...
const styles = {
  ...
  householdCard: { backgroundColor: 'rgba(10,10,10,0.05)' },
  statItem: { padding: 12, borderRadius: 12, backgroundColor: '#F5F5F4' },
  memberItem: { padding: 8, borderRadius: 8, backgroundColor: '#F5F5F4' },
  dangerCard: { backgroundColor: 'rgba(10,10,10,0.05)' },
  dangerButton: { borderColor: '#444444' },
};
