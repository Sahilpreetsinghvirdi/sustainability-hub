// desktop/src/lib/directAI.ts
// Direct, backend-free vision AI. Calls Gemini / OpenAI directly from the
// browser with the user's own API key — no localhost server, works on any
// machine (MSI, web, mobile).
import type {
...
import { loadLocalAISettings } from './api';
...
export function aiConfigured(): boolean {
  const s = loadLocalAISettings();
  return !!(s.gemini_api_key || s.openai_api_key);
}
...
function guessMime(blob: Blob, fallback = 'image/jpeg'): string {
  if (blob.type && blob.type.startsWith('image/')) return blob.type;
  // File.name may still hint at type
  if (blob instanceof File && /\.png$/i.test(blob.name)) return 'image/png';
  return fallback;
}
...
async function callVision(prompt: string, imageB64: string, mimeType: string): Promise<Record<string, unknown>> {
  ...
  if (c.provider === 'gemini') {
    ...
    const res = await fetch(url, {
      method: 'POST',
      headers: { 'x-goog-api-key': loadLocalAISettings().gemini_api_key || '' },
  ...
  const res = await fetch('https://api.openai.com/v1/chat/completions', {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${loadLocalAISettings().openai_api_key || ''}`,
...
// ---------------------------------------------------------------------------
// Normalization helpers (mirror backend, but graceful)
// ---------------------------------------------------------------------------
...
function normHazard(raw: unknown): { level: HazardLevel; score: number; toxins: string[]; health_risks: string[]; environmental_risks: string[] } {
  const d = asDict(raw);
  const valid: HazardLevel[] = ['low', 'medium', 'high', 'critical'];
  let level = asStr(d.level, 'low').toLowerCase() as HazardLevel;
  ...
  return {
    ...
    environmental_risks: asList(d.environmental_risks).slice(0, 15),
  };
}

// ---------------------------------------------------------------------------
// Local heuristic fallbacks (never crash; used when model fails to normalize)
// ---------------------------------------------------------------------------
...
function heuristicWaste(): WasteAnalysisResponse {
  return {
    ...
    overall_hazard: { level: 'low', score: 10, toxins: [], health_risks: [], environmental_risks: [] },
    materials: [],
...
function heuristicAgri(crop: string): AgriAnalysisResponse {
  return {
    ...
    nutrient_profile: { npk: '', organic_matter: null, micronutrients: [], ph_effect: '' },
...
// ---------------------------------------------------------------------------
// Public analysis functions (mirror the old api.ts signatures)
// ---------------------------------------------------------------------------
...
export async function analyzeWasteImage(file: File | Blob, question = '', _fileName = 'capture.jpg'): Promise<WasteAnalysisResponse> {
  ...
  try {
    ...
    // If key present but model failed to normalize, still give a usable (honest) result
...
export async function analyzeFertilizer(file: File | Blob, context: FertilizerContext): Promise<AgriAnalysisResponse> {
  if (!aiConfigured()) throw new Error('No AI key configured. Open Settings → AI Configuration and add a Gemini or OpenAI key.');
  const imageB64 = await blobToBase64(file);
  const mime = guessMime(file);
  ...
  try {
    ...
    let suitability = asStr(verdictRaw.suitability, 'neutral').toLowerCase().replace(/-/g, '_').replace(/ /g, '_') as Suitability;
    const validSuit: Suitability[] = ['beneficial', 'conditionally_beneficial', 'neutral', 'harmful'];
    ...
    return {
      ...
      nutrient_profile: {
        ...
        organic_matter: npkRaw.organic_matter == null ? null : asStr(npkRaw.organic_matter),
...
export async function analyzePlant(file: File | Blob, context: PlantContext = {}): Promise<PlantAnalysisResponse> {
  if (!aiConfigured()) throw new Error('No AI key configured. Open Settings → AI Configuration and add a Gemini or OpenAI key.');
  const imageB64 = await blobToBase64(file);
  const mime = guessMime(file);
  const f = (v?: string) => (v || '').trim() || 'not specified';
  ...
  try {
    ...
    let status = asStr(health.status, 'unknown').toLowerCase().replace(/ /g, '_') as HealthStatus;
    const validHealth: HealthStatus[] = ['healthy', 'stressed', 'diseased', 'critical', 'unknown'];
